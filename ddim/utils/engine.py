...
class DDPMSampler(nn.Module):
    def __init__(self, x_t, t):
        ...
        '''缓冲区与模型的参数（parameters）不同，缓冲区不会参与梯度计算，也就是说，它们不会像权重那样被优化，但它们仍然是模型的一部分。
        注册为缓冲区的张量会随着模型保存和加载而被保留。
        缓冲区通常用于保存那些不需要梯度的状态信息（例如模型的一些常量或缓存），但它们又是模型的一部分，不能被丢弃。'''
        # calculate the cumulative product of $\alpha$ , named $\bar{\alpha_t}$ in paper
        alpha_t = 1.0 - self.beta_t
        alpha_t_bar = torch.cumprod(alpha_t, dim=0)
        alpha_t_bar_prev = F.pad(alpha_t_bar[:-1], (1, 0), value=1.0)  # alpha_t_bar[:-1] 的形状会是 (T-1,)，即去掉最后一个元素后的张量。
        ...
        self.register_buffer("posterior_variance", self.beta_t * (1.0 - alpha_t_bar_prev) / (1.0 - alpha_t_bar)) # sigma^2
    ...
    @torch.no_grad()
    def sample_onestep(self, x_t, t):
        """
        Calculate $x_{t-1}$ according to $x_t$
        """
        # 它的目的是生成一个时间步张量，这个张量的每个元素都表示当前时间步（time_step）
        t = torch.full((x_t.shape[0],), time_step, device=x_t.device, dtype=torch.long)
        mean, var = self.cal_mean_variance(x_t, t)
        
        z = torch.rand_like(x_t) if time_step > 0 else 0
        x_t_minus_one = mean + torch.sqrt(var) * z

        # 5. 检查结果中是否包含 NaN 值，若有则抛出异常
    ...
    def forward(self, x_t, only_return_x_0: bool = True, interval: int = 1, **kwargs):
        ...
        with tqdm(reversed(range(self.T)), colour="#6565b5", total=self.T) as sampling_steps:
            for time_step in sampling_steps:
                x_t = self.sample_onestep(x_t, time_step)
